...
- Список каналов задаётся **строго через Web UI** (`/channels`)
- Cron и веб-кнопка парсят **только активные каналы из БД**
...
**Ключевые решения:**
- **Список каналов контролируется строго через Web UI.** `sync_dialogs()` обновляет `access_hash` для каналов/супергрупп, уже добавленных в БД; новые каналы **не создаёт**.
- `access_hash` кэшируется в БД (обход get_entity())
- `message.post_author` вместо `message.sender` (zero API calls)
- **Постраничный backfill истории:** партиями по 5000 сообщений, курсор хранится в `Channel.metadata_json`
- Сессия БД изолирована от чтения Telegram
- Последовательный парсинг с jitter и adaptive delay (0.5–30 сек)
- Circuit breaker: 3 ошибки → 30 мин паузы
- Cron: `*/5 * * * *` (настраивается в Render Dashboard)

## Как формируется список каналов для парсинга

> **Ни cron, ни веб-кнопка не добавляют каналы автоматически.**

1. Пользователь заходит в `/channels` и добавляет канал/чат (username, t.me-ссылка, Telegram Web URL или numeric ID).
2. `web.py` резолвит сущность через `get_entity()` и сохраняет в `channels` с `is_active = true`.
3. При каждом запуске парсера:
   - `sync_dialogs()` получает все диалоги аккаунта.
   - Для каждого `Channel`/`Chat`, **уже существующего в БД**, обновляет `access_hash`.
   - Каналы, которых нет в БД, **пропускает** (`allow_create=False`).
4. `parse_all()` берёт только `SELECT * FROM channels WHERE is_active = true`.
5. Парсинг выполняется последовательно, 1 канал за раз.

### Поведение `sync_dialogs()` подробно
...
for dialog in await client.get_dialogs():
    ...
    if not isinstance(entity, (Channel, Chat)):
        continue
    # Обновляем access_hash, но НЕ создаём запись
    await _upsert_channel(..., allow_create=False)
```

**Важно:** этот метод не смотрит на `is_active`. Он обновляет `access_hash` даже для отключённых каналов, чтобы при повторном включении парсинг сработал сразу. Но новые диалоги в БД не попадают.

### Последствия ручной очистки БД

Если пользователь удалит все каналы из БД, cron отработает с сообщением:
```
Синхронизировано N каналов/групп
Каналов к парсингу: 0
Нет активных каналов для парсинга
```
Это нормально — нужно заново добавить каналы через Web UI.
...
- **2026-06-18**: **Управление каналами через Web UI**
  - `sync_dialogs()` больше не создаёт каналы автоматически
  - Постраничный backfill истории по 5000 сообщений
  - Фикс сохранения постов без `on_conflict_do_nothing()`
  - Фикс `telegram_id` для супергрупп (`-100` префикс)
  - Обновление карточки сообщения в вебе
