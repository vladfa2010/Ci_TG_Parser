...
> **Список каналов для парсинга строго контролируется через Web UI.**
> Cron и кнопка «🔄 Запустить парсинг» работают **только с активными каналами (`is_active = true`)**, которые вы сами добавили.

`sync_dialogs()` при старте:
1. Получает все диалоги через `get_dialogs()`
2. Обновляет `access_hash` для каналов **уже в БД**
3. **Пропускает** каналы, которых нет в вашей БД — не создаёт их
