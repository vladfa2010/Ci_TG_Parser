...
| **Парсер** | Последовательный многоканальный парсинг с FloodWait защитой и SIGTERM graceful shutdown |
...
[MultiChannelParser.parse_all()]
         ...
         │      a. Пауза 5 секунд (FloodWait защита)
         │      b. Проверка SIGTERM (можно прервать)
         │      c. _lookup_channel() — ищем в БД
         │      d. parse_single_channel() — парсим сообщения
         │
...
| **Semaphore** | 1 | Только 1 канал параллельно |
...
SIGTERM получен
    │
    ├── 1. _is_shutting_down = True
    ├── 2. _shutdown_event.set()
    ├── 3. Текущий канал заканчивается (сохраняет прогресс)
    ├── 4. Следующий канал НЕ начинается
    └── 5. Процесс завершается чисто (~5-10 сек)
```
